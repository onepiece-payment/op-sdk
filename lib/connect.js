...
const goInitiatePayment = (paymentObject) => {
    ...
    if(paymentObject.payment_method === 'alipay') {
        let newAmount = (parseFloat(paymentObject.amount / 100).toFixed(2))
        paymentObject.amount = newAmount;
    }
...
const getAvailablePayments = (pirateToken) => {
    ...
    const magicNum = Math.ceil(Math.random()* 4000);
    const signature = md5(magicNum + pirateToken);
